...
# compute overdispersion parameter by week -- estimate with stan
...
reg_data <- node_attrs %>% 
  ...
  filter(date_max<as.Date('2021-08-01') & date_min>as.Date('2021-01-01')) %>%
  #left_join(npi_data,by=c('date_min'='Date')) %>%
  left_join(rt_iar_file,by=c('date_max'='date')) %>%
  rename(first_vacc_days = days_since_first_vacc, 
         second_vacc_days = days_since_second_vacc) %>%
  mutate(isoweek=as.factor(isoweek),
         vacc_status = 
           as.factor(case_when(first_vacc_complete==FALSE~'unvaccinated',
         ...
         age_group = as.factor(case_when(Age_at_testing < 11 ~ '0-10yrs',
                                         ...
                                         TRUE ~ '>60yrs')))
# simpler model 2
reg_bf3  <- bf( out_degree ~ isoweek + C1_combined_numeric + C2_combined_numeric + 
                  C4_combined_numeric + age_group + vacc_status : weeks_since_vacc + (1|Regionskode ),
                shape ~ ContainmentHealthIndex) 

reg_brm3 <- brm(reg_bf3, 
                data    = reg_data[unique(sort(round(runif(10000,0,61000)))),],  
                prior   = c(prior(normal(0,1),class=b),
                            prior(normal(3.28,1),class=Intercept),
                            prior(gamma(0.2,0.2),class=Intercept, 
                                  dpar = shape,lb=0),
...
saveRDS(reg_brm3,"./stan_output/neg_bin_model_weekly_k_no_shape_priors.rds")
model_outputs <- read_rds("./stan_output/neg_bin_model_weekly_k_no_shape_priors.rds")
abc <- predict(reg_brm3,reg_brm3$data)
...
#Rc_plot   <- plot_Rc(reg_brm3,node_isoweek_dates,rt_iar_file)
#k_plot    <- plot_overdispersion(reg_brm3,node_isoweek_dates)
npi_plot  <- plot_NPI(reg_brm3)
vacc_plot <- plot_vaccination(reg_brm3)
age_plot  <- plot_individual_attributes(reg_brm3)
age_plot
...
plot_k <- k_plot_data  %>% mutate(date_mid=date_min + (date_max-date_min)/2) %>% 
  ggplot(aes(x=date_mid,y=mean, color = parameter)) + geom_line() +
  geom_ribbon(aes(x=date_mid,ymin=`2.5%`, ymax=`97.5%`, fill = parameter,  
                  color = parameter), alpha=0.2, linetype = "dotted") +
